...
for (k in sample(1:1e3,iter)){
  ...
  mlocus_anneal <- function(fseed) {
    ...
    t_anneal <- runif(1,10,1000)
    
    vb_g <- locus(Y = dat_g$phenos, X=dat_g$snps, p0_av = p0_av, link = "identity", user_seed = fseed, list_init = list_init0, full_output = TRUE, anneal = c(1,t_anneal,1))
  ...
  m_vb_g_a <- mclapply(user_seed, mlocus_anneal, mc.cores = nb_cpus)
