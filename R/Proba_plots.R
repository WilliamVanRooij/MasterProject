...
vec_rho <- runif(floor(p/10), min = 0.98, max = 0.99) # correlation coefficients
...
# Definition of functions useful
...
mlocus <- function(fseed) {
  ...
  vb_g <- locus(Y = dat_g$phenos, X=dat_g$snps, p0_av = p0_av, link = "identity", user_seed = fseed, list_init = list_init0, save_init = T, save_hyper=TRUE, anneal = NULL)
  return(vb_g)
}
...
a_mlocus <- function(fseed) {
  ...
  vb_g <- locus(Y = dat_g$phenos, X=dat_g$snps, p0_av = p0_av, link = "identity", user_seed = fseed, list_init = list_init0,save_init = T, save_hyper=TRUE, anneal = anneal)
  return(vb_g)
}
  ...
  if(T){
    ...
    if(TRUE){
      for(i in c(1:length(user_seed))) {
        ...
        b_init <- cbind(b_init, m_vb_g[[i]]$list_init$mu_beta_vb*m_vb_g[[i]]$list_init$gam_vb)
      }
    }
